                            ...
                            {showingServicesForBusiness === business.id && (
                              <div className="mt-3 space-y-2">
                                ...
                                {business.services && business.services.length > 0 ? (
                                  <div className="space-y-2">
                                    {/* Only show parent services or services without a parent */}
                                    {business.services
                                      .filter((service: any) => !service.parentServiceId)
                                      .map((service: any) => (
                                        <div key={service.id}>
                                            ...
                                            <div className="flex-1">
                                              <div className="flex items-center gap-2">
                                                ...
                                                {service.children && service.children.length > 0 && (
                                                  ...
                                                  >
                                                    <svg
                                                      className={`w-4 h-4 transition-transform ${expandedParentServices.has(service.id) ? 'rotate-90' : ''}`}
                                                      fill="none"
                                                      stroke="currentColor"
                                                      viewBox="0 0 24 24"
                                                    >
                                          ...
                                          {service.children && service.children.length > 0 && expandedParentServices.has(service.id) && (
                                            <div className="ml-6 mt-2 space-y-2">
                                              {service.children.map((child: any) => (
                                                <div
                                                  key={child.id}
                                                  className="flex items-center justify-between p-2 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-dark-border rounded-lg"
                                                >
                                                  <div className="flex-1">
                                                    ...
                                                    {(child.price || child.duration) && (
                                                      <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">
                                                        {child.price && <span>{child.price}</span>}
                                                        {child.price && child.duration && <span> • </span>}
                                                  ...
                                                  <div className="flex gap-2 ml-3">
                                                    ...
                                                    <button
                                                      ...
                                                      disabled={deletingServiceId === child.id}
                                                      className="px-2 py-1 bg-red-100 dark:bg-red-900/20 text-red-800 dark:text-red-400 hover:bg-red-200 dark:hover:bg-red-900/30 rounded text-xs font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                                      title="Delete variation"
                                                    >
                                                      {deletingServiceId === child.id ? (
                                                        <svg className="w-3 h-3 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                                                        </svg>
                                                      ) : (
