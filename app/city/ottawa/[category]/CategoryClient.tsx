  ...
  return (
    <div className="min-h-screen bg-white dark:bg-dark-bg flex flex-col">
      ...
      <div className="sr-only">
        <h1>
          {selectedSubcategory
            ? `${subcategories.find((s) => s.id === selectedSubcategory)?.name} in Ottawa`
            : `${categoryName} in Ottawa`}
        </h1>
        <p>
          {selectedSubcategory
            ? `Find verified ${subcategories.find((s) => s.id === selectedSubcategory)?.name.toLowerCase()} businesses in Ottawa. Browse local businesses, read authentic reviews, and connect instantly with top-rated services.`
            : `Welcome to Ottawa's premier ${categoryName.toLowerCase()} directory. Discover verified businesses, connect with trusted local services, compare options, and book instantly.`}
        </p>
      </div>
