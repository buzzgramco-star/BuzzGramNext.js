...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // Fetch city by slug
  const cityResponse = await fetch(`${API_BASE}/cities/by-slug/calgary`, {
    next: { revalidate: 300 },
  });
  const cityData = await cityResponse.json();
  const city = cityData.success ? cityData.data : null;
  const cityName = city?.name || 'Calgary';
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `https://buzz-gram-next-js.vercel.app/city/calgary/${categorySlug}/${subcategorySlug}`,
      siteName: 'BuzzGram',
      locale: 'en_CA',
      type: 'website',
    },
    ...
    alternates: {
      canonical: `https://buzz-gram-next-js.vercel.app/city/calgary/${categorySlug}/${subcategorySlug}`,
    },
  };
}
...
export default async function SubcategoryLayout({ params, children }: Props) {
  ...
  const collectionPageSchema = {
    ...
    '@type': 'CollectionPage',
    '@id': `https://buzz-gram-next-js.vercel.app/city/calgary/${categorySlug}/${subcategorySlug}`,
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 2,
        name: cityName,
        item: 'https://buzz-gram-next-js.vercel.app/city/calgary',
      },
      {
        '@type': 'ListItem',
        position: 3,
        name: categoryName,
        item: `https://buzz-gram-next-js.vercel.app/city/calgary/${categorySlug}`,
      },
      {
        '@type': 'ListItem',
        position: 4,
        name: subcategoryName,
        item: `https://buzz-gram-next-js.vercel.app/city/calgary/${categorySlug}/${subcategorySlug}`,
      },
    ],
  };
