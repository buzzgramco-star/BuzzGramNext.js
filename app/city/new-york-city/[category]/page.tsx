...
export default async function CategoryPage({ params }: Props) {
  try {
    ...
    // Fetch all data server-side in parallel (New York City city ID: 31)
    const [businesses, subcategories] = await Promise.all([
      getBusinesses({ cityId: 32, categoryId }),
      getSubcategories(),
    ]);
    ...
    console.error('[New York City Category Page] Error:', error);
    return (
      <>
        <Suspense fallback={<div className="h-16" />}>
          <Header />
