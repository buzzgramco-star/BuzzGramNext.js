...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // Fetch city by slug
  const cityResponse = await fetch(`${API_BASE}/cities/by-slug/vancouver`, {
    next: { revalidate: 300 },
  });
  const cityData = await cityResponse.json();
  const city = cityData.success ? cityData.data : null;
  const cityName = city?.name || 'Vancouver';
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `https://buzz-gram-next-js.vercel.app/city/vancouver/${categorySlug}/${subcategorySlug}`,
      siteName: 'BuzzGram',
      locale: 'en_CA',
      type: 'website',
    },
    ...
    alternates: {
      canonical: `https://buzz-gram-next-js.vercel.app/city/vancouver/${categorySlug}/${subcategorySlug}`,
    },
  };
}
...
export default async function SubcategoryLayout({ params, children }: Props) {
  ...
  const collectionPageSchema = {
    ...
    '@type': 'CollectionPage',
    '@id': `https://buzz-gram-next-js.vercel.app/city/vancouver/${categorySlug}/${subcategorySlug}`,
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 2,
        name: cityName,
        item: 'https://buzz-gram-next-js.vercel.app/city/vancouver',
      },
      {
        '@type': 'ListItem',
        position: 3,
        name: categoryName,
        item: `https://buzz-gram-next-js.vercel.app/city/vancouver/${categorySlug}`,
      },
      {
        '@type': 'ListItem',
        position: 4,
        name: subcategoryName,
        item: `https://buzz-gram-next-js.vercel.app/city/vancouver/${categorySlug}/${subcategorySlug}`,
      },
    ],
  };
