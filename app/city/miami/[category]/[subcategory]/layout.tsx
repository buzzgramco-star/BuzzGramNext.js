...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // Fetch city by slug
  const cityResponse = await fetch(`${API_BASE}/cities/by-slug/miami`, {
    next: { revalidate: 300 },
  });
  const cityData = await cityResponse.json();
  const city = cityData.success ? cityData.data : null;
  const cityName = city?.name || 'Miami';
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `https://buzz-gram-next-js.vercel.app/city/miami/${categorySlug}/${subcategorySlug}`,
      siteName: 'BuzzGram',
      locale: 'en_US',
      type: 'website',
    },
    ...
    alternates: {
      canonical: `https://buzz-gram-next-js.vercel.app/city/miami/${categorySlug}/${subcategorySlug}`,
    },
  };
}
...
export default async function SubcategoryLayout({ params, children }: Props) {
  ...
  const collectionPageSchema = {
    ...
    '@type': 'CollectionPage',
    '@id': `https://buzz-gram-next-js.vercel.app/city/miami/${categorySlug}/${subcategorySlug}`,
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 2,
        name: cityName,
        item: 'https://buzz-gram-next-js.vercel.app/city/miami',
      },
      {
        '@type': 'ListItem',
        position: 3,
        name: categoryName,
        item: `https://buzz-gram-next-js.vercel.app/city/miami/${categorySlug}`,
      },
      {
        '@type': 'ListItem',
        position: 4,
        name: subcategoryName,
        item: `https://buzz-gram-next-js.vercel.app/city/miami/${categorySlug}/${subcategorySlug}`,
      },
    ],
  };
